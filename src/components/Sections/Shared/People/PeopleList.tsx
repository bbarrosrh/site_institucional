...
import type { Person } from "@/sanity/schemas/People/data";
        ...
        <div className="flex items-center gap-4 lg:gap-6 rounded-lg bg-tertiary p-3.5 lg:p-6 lg:rounded-2xl">
          {featured.photo ? (
            <img
              src={featured.photo}
              alt=""
              className="size-12 lg:size-16 shrink-0 rounded-md lg:rounded-lg object-cover"
            />
          ) : (
          ...
          <div className="flex flex-col lg:gap-1">
            ...
            <p className="text-white/85 text-sm lg:text-lg">{featured.title}</p>
          </div>
        </div>
          ...
          <div className="grid">
            {people.map((person, index) => (
              <p
                key={person.name}
                className={cn(
                  "col-start-1 row-start-1 leading-relaxed whitespace-pre-line text-white/85 max-lg:text-sm",
          ...
          <div className="flex flex-col-reverse items-start gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
            <div className="grid flex-1">
              {people.map((person, index) => (
                ...
                >
                  {person.location} &bull; {person.education}
                </p>
              ))}
            </div>
            {featured.linkedinHref && (
              <a
              ...
              >
                {person.photo ? (
                  <img
                    src={person.photo}
                    alt=""
                    className="size-12 shrink-0 rounded-full object-cover lg:rounded-lg"
                  />
                ) : (
                ...
                <span className="hidden flex-col lg:flex">
                  ...
                  <span className="text-sm text-white/60">{person.title}</span>
                </span>
              </button>
            );
          })}
